...
@section('page-subtitle', 'Formulir penambahan dokter baru')
...
<div class="space-y-6">
    ...
    <nav class="flex items-center gap-2 text-sm text-slate-500">
        ...
        <span class="text-blue-600 font-medium">Tambah Dokter</span>
    </nav>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        ...
        <div class="px-6 py-5 border-b border-slate-100 bg-gradient-to-r from-blue-50 to-white">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="text-base font-bold text-slate-800">Tambah Dokter Baru</h3>
                    <p class="text-xs text-slate-500 mt-0.5">Isi formulir di bawah untuk menambahkan dokter baru</p>
                </div>
            </div>
        </div>

        {{-- Form Body --}}
        <form action="{{ route('admin.dokter.store') }}" method="POST" enctype="multipart/form-data" class="p-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="bio"
                              ...
                              class="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all duration-150 resize-none @error('bio') border-red-400 bg-red-50 @enderror">{{ old('bio') }}</textarea>
                    @error('bio')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                {{-- Foto --}}
                <div class="md:col-span-2">
                    <label for="foto" class="block text-sm font-semibold text-slate-700 mb-1.5">
                        Foto Dokter
                        <span class="text-slate-400 font-normal">(opsional)</span>
                    </label>
                    <div class="border-2 border-dashed border-slate-200 rounded-lg p-4 hover:border-blue-400 transition-colors duration-150 @error('foto') border-red-400 @enderror">
                        <label for="foto" class="flex flex-col items-center gap-2 cursor-pointer">
                            ...
                            <div class="text-center">
                                <span class="text-sm font-medium text-blue-600">Klik untuk pilih foto</span>
                    ...
                    <div id="preview-container" class="hidden mt-3">
                        ...
                        <p class="text-xs text-slate-400 mt-1">Preview foto</p>
                    </div>
                    @error('foto')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
